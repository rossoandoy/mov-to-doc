...
**業務マニュアルを現場に届ける**場合は **2 + 6（HTML + Cloudflare Workers）** をセットで推奨する。PDF は印刷・配布が必要なときだけ追加。
...
6. **Cloudflare Workers 公開** — HTML を `site/` に配置してデプロイ（Step H）。**Step 2 とセットが最も実用的**（参照動画は R2）。
...
3. **（参照動画あり）** MP4 を R2 にアップロードし、`manual.meta.json` の `referenceVideo.url` を設定する（例: `/videos/<slug>.mp4`）。
4. デプロイ（**wrangler CLI**。Cloudflare MCP に deploy ツールは無い）:
...
5. **MCP による検証**（デプロイ後）:
   - `search_cloudflare_documentation` — Static Assets / R2 設定の確認
   - `workers_list` → `workers_get_worker` — Worker の存在確認
6. 公開 URL（例: `https://<worker-name>.<account>.workers.dev`）を README に記載する。
7. **機密情報**: 公開前にキャプチャ・本文の個人情報をレビューする。後から **Cloudflare Access** で認証を追加可能。
